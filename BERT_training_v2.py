...
def convert_to_dataset(df):
    df = df.reset_index(drop=True)  # Supprimer les colonnes d'index inutiles
    return Dataset.from_pandas(df[["cleaned_claim", "label"]], features=Features({
...
def preprocess_function(examples):
    return tokenizer(examples["cleaned_claim"], truncation=True, padding="max_length", max_length=128)
...
training_args = TrainingArguments(
    ...
    fp16=True if DEVICE == "cuda" else False  # Utilisation du mode float16 sur GPU si disponible
)
...
example_texts = [
    ...
    "The sun has gone into lockdown, which could cause freezing weather.",
